...
demo plugin returns a deterministic catalog from memory; this seeder writes
matching products + Amazon analytics rows into the database so the sourcing
pipeline can scan → match by UPC → evaluate → log BUY/WATCH/AVOID decisions,
...
The first 4 products are hand-curated and produce the exact spread of
outcomes used by tests:
...
Pass ``count`` to seed a larger deterministic set (e.g. 500) generated by
``build_demo_catalog``.
"""
...
from app.plugins.suppliers.demo import DEMO_CATALOG, build_demo_catalog
...
async def seed_sourcing_demo(
    db: AsyncSession,
    count: int | None = None,
) -> dict[str, object]:
    """Insert demo products + analytics; return per-product outcome summary.

    ``count`` (optional) is the number of catalog products to seed. When
    omitted only the 4 hand-curated products are seeded (fast, matches the
    exact test spread). Pass a larger count (e.g. 500) to also generate and
    seed a bigger deterministic set. Any ``count <= 4`` seeds just the
    curated catalog.
    """
    ...
    items = (
        DEMO_CATALOG
        if count is None or count <= len(DEMO_CATALOG)
        else build_demo_catalog(count)
    )

    for item in items:
        # Curated products use their hand-tuned analytics blueprint; generated
        # products carry their own analytics inline.
        blueprint = _ANALYTICS.get(item["supplier_sku"]) or item
